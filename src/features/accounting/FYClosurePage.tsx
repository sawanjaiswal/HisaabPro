...
export default function FYClosurePage() {
  ...
  if (fetchStatus === 'error') {
    return (
      <AppShell>
        <Header title={t.fyClosurePage} backTo={ROUTES.CHART_OF_ACCOUNTS} />
        <PageContainer variant="form" className="space-y-6"><ErrorState title={t.couldNotLoadClosures} message={t.checkConnectionRetry} onRetry={refresh} /></PageContainer>
      </AppShell>
    )
  }
      ...
      <PageContainer variant="form" className="space-y-6">
        ...
        {closures.length === 0 && (
          <EmptyState
            ...
            title={t.noFyClosuresYet}
            description={t.closeFyAbove}
          />
        )}
