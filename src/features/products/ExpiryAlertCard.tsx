...
export function ExpiryAlertCard({ alert }: ExpiryAlertCardProps) {
  ...
  const color = isExpired ? 'var(--color-danger)' : 'var(--color-warning)'
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-3)' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color, background: surface, borderRadius: 'var(--radius-full)', padding: '2px 8px' }}>
          ...
          <div style={{ marginTop: 'var(--space-2)', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 'var(--space-2)' }}>
            <div>
              <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>{t.expiryBatchLabel}</div>
              <div style={{ fontSize: 'var(--text-sm)', fontWeight: 500 }}>{alert.batch.batchNumber}</div>
            </div>
            <div>
              <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>{t.expiryDateLabel}</div>
              <div style={{ fontSize: 'var(--text-sm)', fontWeight: 500, color }}>{formatExpiryDate(alert.batch.expiryDate)}</div>
            </div>
            <div>
              <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>{t.expiryStockLabel}</div>
              <div style={{ fontSize: 'var(--text-sm)', fontWeight: 500 }}>{alert.batch.currentStock} {alert.product.unit.symbol}</div>
            </div>
            <div>
              <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)' }}>Status</div>
              <div style={{ fontSize: 'var(--text-sm)', fontWeight: 500, color }}>{expiryLabel(alert.daysRemaining, alert.alertType)}</div>
            </div>
          </div>
        </div>
      </div>
