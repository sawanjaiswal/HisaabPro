...
export const EWayBillGenerateForm: React.FC<EWayBillGenerateFormProps> = ({
  ...
  const validate = (): boolean => {
    ...
    if (!form.distance || Number(form.distance) <= 0) e.distance = 'Enter distance in km'
    if (form.fromPincode.length !== 6) e.fromPincode = '6-digit pincode required'
    if (form.toPincode.length !== 6) e.toPincode = '6-digit pincode required'
    setErrors(e)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setActionError(err instanceof Error ? err.message : 'Failed to generate e-way bill')
    }
  }

  return (
    <form className="ewb-form" onSubmit={handleSubmit} aria-label="Generate e-way bill form" noValidate>
      {actionError && <p className="compliance-inline-error" role="alert">{actionError}</p>}
      <div className="ewb-form-row">
        <div className="input-group">
          <label className="input-label" htmlFor="ewb-mode">Transport Mode</label>
        ...
        <div className="input-group">
          <label className="input-label" htmlFor="ewb-vtype">Vehicle Type</label>
      ...
      <div className="ewb-form-row">
        <div className="input-group">
          <label className="input-label" htmlFor="ewb-vehicle">Vehicle No.</label>
        ...
        <div className="input-group">
          <label className="input-label" htmlFor="ewb-dist">Distance (km) *</label>
      ...
      <div className="ewb-form-row">
        <div className="input-group">
          <label className="input-label" htmlFor="ewb-from">From Pincode *</label>
        ...
        <div className="input-group">
          <label className="input-label" htmlFor="ewb-to">To Pincode *</label>
      ...
      <div className="input-group">
        <label className="input-label" htmlFor="ewb-transporter">Transporter Name (optional)</label>
