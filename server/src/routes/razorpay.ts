...
// P2.7 — rate-limit webhook to mirror marketing webhooks (600/min/IP).
// Public unsigned-reject path is otherwise a CPU-amplification surface.
const razorpayWebhookRateLimit = createRateLimiter({
  windowMs: 60_000,
  max: 600,
  keyFn: (req) => `rl:rzp:wh:${req.ip}`,
  message: 'Webhook rate limit exceeded',
})

razorpayWebhookRouter.post(
  '/webhook',
  razorpayWebhookRateLimit,
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!secret) {
      // P1.2 — fail closed. Misconfigured prod must NOT accept unsigned payloads.
      // Mirror marketing-aisensy webhook pattern. Dev env should set
      // RAZORPAY_WEBHOOK_SECRET to a stub value (e.g. 'stub-secret-not-real').
      logger.error('razorpay.webhook_secret_missing', {
        message: 'RAZORPAY_WEBHOOK_SECRET not set — refusing webhook (fail-closed)',
      })
