...
**P4 Consistency Sweep (COMPLETE, ratchet via `scripts/enforce-primitives.mjs`):** `rawSelect` 58 → **0** across waves 13–17, `rawTextarea` 30 → **0** in wave 18, `rawInput` 294 → **0** in wave 19, `rawButton` 594 → **0** in wave 20 (322 files; added `variant="none"` to Button + forwardRef so the mechanical wrap preserves caller's className contract, batch `a372261`). Baseline now ZERO across **all** 6 surfaces (`rawButton`, `rawInput`, `rawSelect`, `rawTextarea`, `nativeConfirm`, `missing*State`). Pre-commit refuses regressions.

**P4 semantic-variant upgrade (in progress):** wave-20 was a mechanical wrap that preserved bespoke per-feature classNames. The follow-up sweep migrates `<Button variant="none">` callsites onto real variants (`primary`/`secondary`/`accent`/`destructive`/`ghost`). First slice 2026-05-30 (`d3e64b5`): 14 `className="btn btn-ghost btn-icon"` callsites across 13 marketing/BOM/production-runs pages → `variant="ghost" className="btn-icon"` (cva emits identical class string — no visual change). **Remaining 603 callsites carry bespoke per-feature CSS (`fab`, `calculator-key`, `pos-back-btn`, `pill-tab`, `expense-add-btn`, etc.) — those need per-page design judgment, not a mechanical sweep (cva would emit `btn btn-ghost fab` and fight the bespoke styles).** Page-by-page workstream.
