...
- 2026-05-26 19:32 | W4 | Remaining manual smokes deferred to device (browser dev server + Capacitor build): offline mutation queue drain, PIN gate lockout/recovery, subscription trial→expired→reactivated, Lighthouse ≥90 on landing.
- 2026-05-26 20:00 | W4 | **Server tests: 1029/1029 passing** (`81fb937`). Fixed 4 root causes: (1) global mock for `replay-protection` middleware (was 400-ing every request on missing nonce headers), (2) global mock for `subscription-gate` factories (was 402-ing paid-tier routes via FREE default), (3) `vitest.config.ts` env vars `FEATURE_STAFF_HR=true` + `FEATURE_STAFF_HR_COHORT_PCT=100` + same for `DATA_IMPORT` (require-feature middleware's `cohortPercent ≤ 0 → false` masked enabled=true), (4) `commit-dispatcher.test.ts` updated from PR-C1 scaffold-throw assertion to PR-C3 returns-function assertion.
- 2026-05-26 20:08 | W4 | **Frontend tests: 1286/1306 passing** (`d55d40b`). Bucket A (no-QueryClient, 257 tests across 43 files) cleared via shared `createTestWrapper()` helper at `src/test/query-wrapper.tsx` (QueryClientProvider + MemoryRouter) wired into every `renderHook(fn, { wrapper })` site. tsc clean, enforce clean.
- 2026-05-26 20:08 | W4b | **Deferred FE test-contract drift (20 tests, 15 files)** — separate from the QueryClient root cause; these are real source/test contract bugs that the QueryClient failure was masking. Breakdown:
    - 7× "handleDelete optimistically removes" (useParties / useInvoices / usePayments / useProducts / useShareLedger×3 / useUnitManager) — react-query cache vs old local-state semantics; tests check `.current.list.length` but data flows through query cache.
    - 4× form `handleSubmit` error path (useInvoiceForm / usePaymentForm / useProductForm / useTemplateForm) — `useMutation` swallows throws differently than try/catch; tests expect raw `throw` propagation.
    - 2× "prevents concurrent" (useEInvoiceActions / useEWayBillActions) — old local-guard semantics vs `mutation.isPending`.
    - 2× useDashboard error/abort — react-query error-shape mismatch.
    - 2× `business.utils.getBusinessInitials` — source returns 1 char, test expects 2 chars (pre-existing source bug, unrelated to react-query migration).
    - 1× useOnlineStatus jsdom navigation — `Not implemented: navigation to another Document` from heartbeat probe.
    - 2× misc assertions.
  Not a launch blocker (1306/1326 = 98.5%, all critical paths green, server 1029/1029). Will be cleared post-beta during W4b test-contract sweep.
- 2026-05-27 21:55 | W4b | **Cleared: 1306/1306 FE tests passing** (`c728af5`). Root causes were a mix of real source bugs (4) and stale tests (16): (a) form `handleSubmit` rethrew mutateAsync rejections after `onError` already toasted — wrapped in try/catch across 4 hooks; (b) `useDashboard` treated AbortError as a hard error — now surfaces as still-loading (React Strict Mode + rapid remount aborts the first request); (c) `getBusinessInitials` returned 1 char for single-word names; (d) `checkOnlineNow` test asserted false for non-ok responses but the hook deliberately treats any HTTP response as reachable (only network/timeout = offline) — test renamed and flipped; (e) handleDelete optimistic-removal tests needed `await waitFor()` wraps for react-query cache propagation; (f) shared module-scoped QueryClient leaked cached data across tests in `useShareLedger` and `useDashboard` — moved `createTestWrapper()` into `beforeEach`. tsc clean, enforce clean.
